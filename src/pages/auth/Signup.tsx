    ...
    <div className="min-h-screen flex items-center justify-center bg-[#020617] p-6 relative overflow-hidden font-sans">
      ...
      >
        ...
        <div className="bg-[#0f172a]/40 border border-white/10 backdrop-blur-3xl rounded-[3rem] p-12 md:p-16 shadow-2xl relative overflow-hidden">
          ...
          <Tabs value={role} className="mb-10" onValueChange={(v) => setRoleState(v as any)}>
            <TabsList className="grid w-full grid-cols-2 bg-white/5 p-1.5 h-16 rounded-2xl border border-white/10">
              <TabsTrigger 
                value="creator" 
                className="rounded-xl data-[state=active]:bg-primary data-[state=active]:text-white transition-all flex items-center gap-3 text-xs font-black uppercase tracking-widest"
              >
          ...
          <form onSubmit={handleSignup} className="space-y-8">
            ...
            <div className="space-y-3">
              <Label htmlFor="password" className="text-[11px] font-black uppercase tracking-[0.3em] text-white/40 ml-1">Access Key (Password)</Label>
              <div className="relative group">
          ...
          <p className="mt-12 text-center text-sm font-medium text-white/30">
            Already have an account? <Link to="/login" className="text-primary font-black uppercase tracking-widest text-xs ml-2 hover:underline">Log in</Link>
          </p>
        </div>
      </motion.div>
    </div>
